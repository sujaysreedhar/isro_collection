...
<head>
    ...
    <style>
        ...
        body { font-family: 'Plus Jakarta Sans', sans-serif; background-color: #111827; color: #f9fafb; }
        .glass-header { background: rgba(31, 41, 55, 0.85); backdrop-filter: blur(12px); border-bottom: 1px solid rgba(55, 65, 81, 0.8); }
    </style>
    <?= $additionalHead ?? '' ?>
    ...
    <header class="glass-header sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <?php else: ?>
                    <div class="w-10 h-10 bg-gradient-to-br from-dark-700 to-dark-900 rounded-xl shadow-lg flex items-center justify-center text-white font-bold text-xl">
            ...
            <div class="flex-1 max-w-xl mx-8 hidden md:block">
                <form action="<?= SITE_URL ?>/search.php" method="GET" class="relative group">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <svg class="h-5 w-5 text-slate-400 group-focus-within:text-dark-500 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                    </div>
                    <input type="text" name="q" class="block w-full pl-10 pr-3 py-2.5 border border-dark-700 rounded-full bg-dark-800 placeholder-dark-400 focus:outline-none focus:ring-2 focus:ring-dark-500/20 focus:border-dark-500 focus:bg-dark-700 text-sm transition-all duration-300 shadow-sm" placeholder="Search archives, documents, artifacts..." />
                </form>
            </div>
