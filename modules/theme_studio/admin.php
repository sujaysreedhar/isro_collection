...
<div class="mb-6 flex flex-wrap items-start justify-between gap-4">
    <div>
        ...
        <p class="text-sm text-gray-500 mt-1">Customise colors, fonts, and home page layout for the
            <strong>Custom</strong> theme.</p>
    </div>
...
<div id="ts-toast"
    class="hidden fixed bottom-6 right-6 z-50 px-5 py-3 rounded-xl shadow-xl text-sm font-semibold text-white bg-green-600 transition-all">
    ✓ Saved!</div>
<div id="ts-errtoast"
    class="hidden fixed bottom-6 right-6 z-50 px-5 py-3 rounded-xl shadow-xl text-sm font-semibold text-white bg-red-500 transition-all">
    Error saving.</div>
        ...
        <div id="ts-tab-colors" class="ts-tab-panel active">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div>
                    ...
                    foreach ($colorFields as $f):
                        ...
                        <div class="ts-color-row">
                            <div class="ts-pickr-btn" id="cp-<?= $f['key'] ?>" data-key="<?= $f['key'] ?>"
                                data-value="<?= htmlspecialchars($val) ?>"></div>
                            <span class="ts-color-label"><?= $f['label'] ?></span>
                            <input type="text" class="ts-color-hex" id="hex-<?= $f['key'] ?>" data-key="<?= $f['key'] ?>"
                                value="<?= htmlspecialchars($val) ?>" oninput="fromHexInput('<?= $f['key'] ?>',this.value)"
                                maxlength="7">
                        </div>
                    <?php endforeach; ?>
                    ...
                    ?>
                    <div class="ts-color-row">
                        <div class="ts-pickr-btn" id="cp-theme_studio_hero_overlay_color"
                            data-key="theme_studio_hero_overlay_color"
                            data-value="<?= htmlspecialchars($overlayColor) ?>"></div>
                        <span class="ts-color-label">Overlay / shadow color <span class="text-xs text-gray-400">(tint
                                over hero image)</span></span>
                        <input type="text" class="ts-color-hex" id="hex-theme_studio_hero_overlay_color"
                            data-key="theme_studio_hero_overlay_color" value="<?= htmlspecialchars($overlayColor) ?>"
                            oninput="fromHexInput('theme_studio_hero_overlay_color',this.value)" maxlength="7">
                    </div>
                    <div class="ts-color-row items-center">
                        <span class="ts-color-label">Overlay opacity</span>
                        <div class="flex items-center gap-2 ml-auto">
                            <input type="range" id="ts-overlay-opacity" min="0" max="100"
                                value="<?= (int) $overlayOpacity ?>" class="w-28 accent-blue-600"
                                oninput="document.getElementById('ts-overlay-opacity-val').textContent=this.value+'%'">
                            <span class="text-sm font-mono text-gray-600 w-10"
                                id="ts-overlay-opacity-val"><?= (int) $overlayOpacity ?>%</span>
                        </div>
                    </div>
                ...
                <div>
                    ...
                    <div id="ts-color-preview" class="rounded-xl overflow-hidden shadow border border-gray-200"
                        ...
                        <div id="pv-header" class="px-5 py-3 flex items-center justify-between"
                            ...
                            <span class="font-bold text-sm" id="pv-title"
                                style="color:<?= $s('theme_studio_color_primary', '#111827') ?>"><?= SITE_TITLE ?></span>
                            <span class="text-xs px-3 py-1 rounded-full text-white" id="pv-btn"
                                style="background:<?= $s('theme_studio_color_accent', '#2563eb') ?>">Search</span>
                        </div>
                        <!-- Fake hero -->
                        <div id="pv-hero" class="px-5 py-8"
                            style="background:<?= $s('theme_studio_color_hero_bg', '#fff') ?>">
                            <div class="font-extrabold text-xl mb-2" id="pv-heading"
                                style="color:<?= $s('theme_studio_color_primary', '#111827') ?>"><?= SITE_TITLE ?></div>
                            <div class="text-sm mb-4" id="pv-sub"
                                style="color:<?= $s('theme_studio_color_text_muted', '#6b7280') ?>">Explore the
                                collection</div>
                            <div class="ts-preview-bar w-1/2" id="pv-bar"
                                style="background:<?= $s('theme_studio_color_accent', '#2563eb') ?>"></div>
                        </div>
                        <!-- Fake body -->
                        <div id="pv-body" class="px-5 py-6"
                            style="background:<?= $s('theme_studio_color_bg', '#f9fafb') ?>">
                            <div class="grid grid-cols-2 gap-3">
                                <?php for ($i = 0; $i < 4; $i++): ?>
                                    <div class="rounded-lg p-3" id="pv-card-<?= $i ?>"
                                        ...
                                        <div class="h-2 rounded mb-2 w-3/4"
                                            style="background:<?= $s('theme_studio_color_border', '#e5e7eb') ?>"></div>
                                        <div class="h-2 rounded w-1/2"
                                            style="background:<?= $s('theme_studio_color_text_muted', '#6b7280') ?>;opacity:.4">
                                        </div>
                                    </div>
                                <?php endfor; ?>
                            </div>
                        ...
                        <!-- Fake footer -->
                        <div id="pv-footer" class="px-5 py-3 text-xs"
                            style="background:<?= $s('theme_studio_color_footer_bg', '#111827') ?>;color:<?= $s('theme_studio_color_footer_text', '#9ca3af') ?>">
                            © <?= date('Y') ?> <?= SITE_TITLE ?>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        <?php foreach ($presets as $pr): ?>
                            <button
                                class="px-3 py-1.5 text-xs font-semibold rounded-lg border border-gray-200 hover:border-blue-300 hover:bg-blue-50 transition-colors"
                                onclick="applyPreset(<?= htmlspecialchars(json_encode($pr)) ?>)">
                                <span class="inline-block w-3 h-3 rounded-full mr-1.5 align-middle"
                                    style="background:<?= $pr['accent'] ?>"></span>
                ...
                <div>
                    ...
                    <div class="space-y-3 mt-1">
                        <?php foreach ($fontPairs as $pair):
                            ...
                            ?>
                            <label
                                class="flex items-start gap-3 p-3 rounded-xl border border-gray-200 cursor-pointer hover:border-blue-300 hover:bg-blue-50/50 transition-all has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50">
                                <input type="radio" name="font_pair" value="<?= htmlspecialchars($val) ?>" <?= $checked ?>
                                    onchange="applyFontPair('<?= $pair['body'] ?>','<?= $pair['heading'] ?>')"
                                    class="mt-0.5 accent-blue-600">
                                <div>
                                    <div class="text-sm font-semibold text-gray-800"><?= htmlspecialchars($pair['label']) ?>
                                    </div>
                                    <div class="text-xs text-gray-500">Body: <?= htmlspecialchars($pair['body']) ?> ·
                                        Heading: <?= htmlspecialchars($pair['heading']) ?></div>
                                </div>
                            </label>
                        <?php endforeach; ?>
                    </div>
                ...
                <div>
                    ...
                    <div class="flex gap-3 flex-wrap mt-1">
                        <?php foreach (['0px' => 'Sharp', '0.25rem' => 'Soft', '0.5rem' => 'Rounded', '0.75rem' => 'Rounder', '1rem' => 'Pill-ish'] as $rv => $rl):
                            ...
                            ?>
                            <label class="ts-radio-card <?= ($cur === $rv ? 'selected' : '') ?> flex-none" style="min-width:5rem"
                                data-group="radius"
                    ...
                    <input type="hidden" id="ts-radius"
                        value="<?= htmlspecialchars($s('theme_studio_border_radius', '0.5rem')) ?>">
                </div>
            </div>
        </div><!-- /typography tab -->
        ...
        <div id="ts-tab-layout" class="ts-tab-panel">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                <div class="space-y-8">
                    ...
                    <!-- Hero tagline -->
                    <div>
                        <label class="ts-section-title block">Hero H1 Text <span
                                class="text-gray-400 font-normal normal-case text-xs">(first line above site
                                title)</span></label>
                        ...
                        <p class="mt-1 text-xs text-gray-400">In Split layout this appears as a second line in accent
                            color below the h1 text.</p>
                    </div>
                    ...
                    <div class="p-4 rounded-xl border border-gray-200 space-y-3">
                        <p class="ts-section-title" style="margin-top:0">Hero Text Colors <span
                                class="text-gray-400 font-normal normal-case text-xs">(leave blank to use theme
                                defaults)</span></p>
                        <?php
                        ...
                        <div class="ts-color-row">
                            ...
                            <input type="text" class="ts-color-hex" id="hex-theme_studio_hero_text_color"
                                data-key="theme_studio_hero_text_color"
                        ...
                        <div class="ts-color-row">
                            ...
                            <input type="text" class="ts-color-hex" id="hex-theme_studio_hero_tagline_color"
                                data-key="theme_studio_hero_tagline_color"
                        ...
                        <div class="ts-color-row">
                            ...
                            <input type="text" class="ts-color-hex" id="hex-theme_studio_hero_accent_color"
                                data-key="theme_studio_hero_accent_color"
                        ...
                        <p class="text-xs text-gray-400 mt-1">Tip: use white (#ffffff) when the hero has a dark image
                            for readability.</p>
                    </div>
                    ...
                    <div>
                        ...
                        <div id="ts-hero-img-box"
                            ...
                            <?php if ($heroImgUrl): ?>
                                ...
                                <div
                                    ...
                                    <span class="text-white text-sm font-semibold bg-black/40 px-3 py-1 rounded-lg">Click to
                                        change</span>
                                </div>
                            <?php else: ?>
                        ...
                        <input type="file" id="ts-hero-file" accept="image/*" class="hidden"
                            onchange="uploadHeroImage(this)">
                        <?php if ($heroImgUrl): ?>
                            <button onclick="removeHeroImage()"
                                class="mt-2 text-xs text-red-500 hover:text-red-700 font-medium">Remove hero image</button>
                        <?php endif; ?>
                        <p class="mt-1.5 text-xs text-gray-400">Used in Split (right column) and Centered (background)
                            hero layouts.</p>
                    </div>
                        ...
                        <div class="flex gap-3 mt-1">
                            <?php foreach ($colOptions as $cols => $opt): ?>
                                <label class="ts-radio-card flex-1 <?= ($curCols === $cols ? 'selected' : '') ?>"
                                    ...
                                    <div class="grid gap-1 mx-auto mb-2"
                                        style="grid-template-columns:repeat(<?= $cols ?>,1fr);width:<?= ((int) $cols * 14) ?>px">
                                        <?php for ($i = 0; $i < (int) $cols; $i++): ?>
                                            <div class="rounded-sm"
                                                style="height:18px;background:var(--color-accent,#2563eb);opacity:<?= $curCols === $cols ? '1' : '.4' ?>">
                                            </div>
                                        <?php endfor; ?>
                                    </div>
                    ...
                    <div>
                        ...
                        <select id="ts-featured-count"
                            ...
                            <?php foreach (['3', '4','6', '8', '9', '12'] as $n): ?>
                                <option value="<?= $n ?>" <?= $s('theme_studio_featured_count', '6') === $n ? 'selected' : '' ?>>
                                    <?= $n ?> items</option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <!-- Footer text -->
                    <div>
                        <label class="ts-section-title block">Footer Text <span
                                class="text-gray-400 font-normal normal-case text-xs">(leave blank to use default
                        ...
                        <p class="mt-1 text-xs text-gray-400">Supports line breaks. Shown left-aligned in the footer.
                        </p>
                    </div>
                ...
                <div>
                    ...
                    <div class="rounded-xl overflow-hidden shadow border border-gray-200 bg-white">
                        ...
                        <div id="pv-layout-hero" class="px-4 py-6 text-center border-b border-gray-100"
                            ...
                            <div class="mt-3 h-6 bg-gray-200 rounded-lg w-3/4 mx-auto flex items-center px-2">
                                ...
                                <div class="ml-1.5 px-2 py-0.5 rounded"
                                    style="background:var(--color-accent,#2563eb);width:24px;height:14px"></div>
                            </div>
                        </div>
